...
- **The live Zoom session is for course material only. Technical questions about running `RStudio` will *not* be addressed at the live online session. Please email the workshop presenters beforehand if you need help with technical issues related to `R`.**
</div>
...
These flowcharts do not encourage much in the way of thought about the underlying structures, models and processes of these methods, and imply that the small collection of tests will do for all circumstances (which is absolutely not true). They tend not to present any conceptual framework that helps researchers through the inevitable cases where the formulaic application of these flowcharts is inappropriate for the question being asked of the research. These classical statistical methods also tend to be inflexible and fragile: unable to adapt to novel contexts not met by their assumptions; and failing in unpredictable ways when they meet those contexts.

These novel contexts arise fairly frequently in research.

Statistics is toolbox containing many tools that will make your life easier if you know when they should be used. In this workshop, we aim to give you some insight and tools to help you understand common statistical methods you might encounter, why they are used and - occasionally - why they perhaps should not have been used. One of our goals is to encourage you to look past a rote application of "The Null Ritual" (coined in [Gigerenzer (2004) *J. Socio-Econ.* doi:10.1016/j.socec.2004.09.033](https://doi.org/10.1016/j.socec.2004.09.033)), as though it's a hammer and every problem is a nail to be hammered down. The Null Ritual runs like this: 
...
We are instead trying to introduce some statistical thinking, in terms of understanding the bases of common statistical tools, their applications, and their limitations. This will help you determine whether a particular approach can answer your question. Sometimes, you might find you need to use a new tool.
...
- *prediction* questions also need to determine a similar relationship between *explanatory* and *response* variables, but they then seek to use those relationships to *predict* responses of individuals on the basis of measurements of the explanatory variables that might be made in the future.
...
Firstly, although we measure data in our experiments, some statistical approaches do not use the raw data directly to calculate their result. Instead, they use the data to calculate values that are *parameters* of a *distribution* (e.g. *mean* and *standard deviation*), such as a Normal or Poisson distribution. The corresponding distribution is an idealised *model* of the measurements, and it simplifies calculations to use this model to represent our data, instead of the actual individual datapoints. 
...
The means and standard deviations were parameters that described an idealised model of your data, in place of using the raw measurements themselves. The parameters may then have been used to answer an *understanding* question: "should we believe that these two distributions have different mean values?"
</div>

A key concern in statistics is ensuring that the choice of model distribution to represent an experimental dataset is appropriate, and we will spend some time in the workshop notebooks exploring how common distributions represent data. In Figure \@ref(fig:model-normal) below, we can see a curve describing the expected distribution of frequencies of values for a Normal distribution, and a single dataset of 200 datapoints sampled from that distribution. Note that the sampled data does not follow the shape of the curve exactly.
...
This will allow you to explore the way in which the parameters of a statistical distribution alter its shape, and therefore how well it appears to represent measured data.
</div>
...
Much of statistical analysis of *relationships* between variables is about estimating the values of parameters for these models, and exploring the implications of the resulting parametrised models.
...
Although we will not dive into theory or mathematics in detail, you should understand that these methods are mathematical, and are not constructed in an *ad hoc* way. There is sound theory behind them, even though we might criticise their application.
...
3. **Use version control**. Version control is a way to ensure a "chain of custody" of analyses and data, like preserving evidence in a criminal case. It is best-practice for computational work of all kinds, and is naturally supported in `R` (though we'll not talk about it in detail, here).
