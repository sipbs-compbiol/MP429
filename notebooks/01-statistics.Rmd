...
- You will need to use `RStudio` Desktop, installed on your own computer, for the optional interactive sections in these notebooks
  - For interactive sections of the notebooks you will be asked to:
...
<div id="note">
If you have ever performed a *t*-test, based on the means and standard deviations of two datasets, you were doing exactly this! The means and standard deviations were parameters that described a model of your data, in place of using the actual measurements themselves.
</div>

A key concern in statistics is ensuring that the choice of model distribution to represent an experimental dataset is appropriate, and we will spend some time in the workshop notes exploring this.
...
dfm_hist = data.frame(vals=rnorm(200, mean=mu, sd=sd))

p = ggplot(dfm_hist, aes(x=vals))
p = p + geom_histogram(aes(y=..density..),
                       breaks=breaks,
                       fill="cornflowerblue")
p = p + stat_function(fun=dnorm, args=list(mean=mu, sd=sd),
                      geom="line")
p = p + annotate("segment",
                 x=c(mu-2*sd, mu-sd, mu+sd, mu+2*sd), xend=c(mu-2*sd, mu-sd, mu+sd, mu+2*sd),
...
p = p + xlim(mu - 3 * sd, mu + 3 * sd)
p = p + xlab("measured variable") + ylab("frequency")
p = p + theme(plot.background = element_rect(fill = figbg, color = figbg))
p
```
