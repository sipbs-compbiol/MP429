...
Many other natural random processes are not *continuous*, but have discrete outcomes. For instance, a coin toss can be only Heads or Tails - there are no intermediate values. A coin toss is an event that can have only one of two possible outcomes, and many experiments are set up in this way - with an outcome that is either "success" (Heads) or "failure" (Tails). The statistics of coin tosses are a good substitute for the statistics of these kinds of experiments.
