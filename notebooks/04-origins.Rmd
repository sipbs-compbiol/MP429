...
- [4a. Where Does the Normal Distribution Come From? **INTERACTIVE SESSION**](https://mp429.shinyapps.io/04a-generate_normal/)
...
Many other natural random processes (and experimental observations) are not *continuous*, but instead have discrete outcomes. For instance, a coin toss can be only Heads or Tails - there are no intermediate values. Many experiments are set up in a similar way, to have an outcome that is either "success" (Heads) or "failure" (Tails). The statistics of coin tosses are a good substitute for the statistics of these kinds of experiments.
...
- The probability of any single experiment being a success is represented as $p$, and the probability of failure is $q = 1 - p$.
...
For a fair coin, $p = q = 0.5$, and we can flip it $n=30$ times, to obtain three different runs of trials, calling `T` for Tails, and `H` for Heads:
...
```
<div id="note">
Please take some time to explore how the Binomial Distribution arises naturally out of performing many runs of Bernoulli Trials.

- [4b. Where Do the Binomial and Negative Binomial Distributions Come From? **INTERACTIVE SESSION**](https://mp429.shinyapps.io/04b-bernoulli/)

In this session, you can explore the way in which the variation in success probability and number of trials affects these statistical distributions.
</div>

As you will have seen from the interactive session, the Binomial Distribution can vary in shape, and the mean, median and mode of the distribution can take different values in the same distribution.
...
<div id="note">
Please take some time to explore how the Negative Binomial Distribution arises naturally out of performing many runs of Bernoulli Trials.
...
As you will have seen from the interactive session, the Negative Binomial Distribution can take a number of shapes, and the mean, median and mode can be quite different.
...
<div id="warning">
The "tail" of the Exponential distribution can be quite long. A distribution with a small average waiting time may still contain several representatives with very long waiting times.
</div>
...
<div id="note">
There is a single parameter for this process, and it is the same one as for the Exponential distribution: the average *rate* at which events occur, 𝛌. (You can think of this as the number of events per unit time)
</div>


```{r model-pois, echo=FALSE, fig.cap="Two hundred 'measured' data values (histogram) and the corresponding Poisson distribution model (dots) generating those values. The modelled distribution is entirely described by the rate parameter 𝛌=3. The frequencies of observed data values don't match the curve exactly; as for the Normal distribution the model simplifies our representation of the 'noisy' data. The dashed line represents the expected mean value of the model (𝛌)"}
lambda = 3
breaks = 0:15
expect = lambda
...
<div id="note">
Please take some time to explore how the Poisson Distribution arises naturally out of an Exponential Distribution.

- [4c. Where Does the Poisson Distribution Come From? **INTERACTIVE SESSION**](https://mp429.shinyapps.io/04c-poisson/)

In this session, you can explore the way in which the variation in rate parameter affects both Exponential and Poisson Distributions.
</div>
